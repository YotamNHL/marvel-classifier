...
class Evaluation:
    ...
    def plot_cv_precision_recall(self, clf, n_folds, n_repeats, X, y, random_state=0, add_anomaly_score=False,
                                 stacking=False,
                                 add_cluster=False, extra_tree_feature=False, trial_feature=False):
        ...
        for train, test in cv.split(X, y):
            ...
            accuracies.append(accuracy_score(y_pred=y_pred, y_true=y[test]))
        ...
        plt.text(x=0.2, y=0.3, s=r'Mean (f1s = %0.3f $\pm$ %0.2f), Accuracy: %0.3f' % (
            np.mean(f1s), np.std(f1s), np.mean(accuracies)), fontsize=20)
        plt.show()
    ...
    def plot_precision_recall_test(self, y_true, y_scores):
        mean_recall = np.linspace(0, 1, 100)
